...
class AuthenticateUserUseCase:
  ...
  async def execute(self,user_authenticate: AuthenticateUserInputDTO) -> AuthenticateUserOutputDTO:
    user = await self.user_repository.find_by_cpf(user_authenticate.cpf)
    
    if not user:
      raise UserNotFoundByCpfError(user_authenticate.cpf)
